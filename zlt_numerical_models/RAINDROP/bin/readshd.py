...
def readshd( filename=None, xs=None, ys=None ):
    ...
    recl  = int( fromfile( fid, int32, 1 )[0] )
    title = fid.read(80)
    fid.seek( 4*recl )
    PlotType = fid.read(10)
    fid.seek( 2*4*recl ); # reposition to end of second record
    Nfreq  = int(   fromfile( fid, int32  , 1 )[0] )
    Ntheta = int(   fromfile( fid, int32  , 1 )[0] )
    Nsx    = int(   fromfile( fid, int32  , 1 )[0] )
    Nsy    = int(   fromfile( fid, int32  , 1 )[0] )
    Nsd    = int(   fromfile( fid, int32  , 1 )[0] )
    Nrd    = int(   fromfile( fid, int32  , 1 )[0] )
    Nrr    = int(   fromfile( fid, int32  , 1 )[0] ) 
    atten  = float( fromfile( fid, float32, 1 )[0] )
