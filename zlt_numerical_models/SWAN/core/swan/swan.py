...
class SWAN:
    ...
    def init_swan_from_template(self, model_id: str, target_folder: str, config: SwanConfig):
        ...
        if self._config.task.is_stationary:
            config_template_name = 'config_template_stat.swn'
        else:
    ...
    def run(self, is_parallel=False, num_cores=None, verbose=True):
        ...
        params = {'shell': True}
        if not verbose:
            params['stdout'] = subprocess.PIPE #subprocess.DEVNULL

        if is_parallel:
            subprocess.run(fr'bash swanrun.sh {self._config_name} {num_cores}', **params)
            print('concluded parallel run \n')
        else:
            subprocess.run(fr'bash swanrun.sh {self._config_name}', **params)
