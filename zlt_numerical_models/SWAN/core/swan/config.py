...
class SwanConfig:
    ...
    @staticmethod
    def generate(task: SimulationTask, grid: Grid, output_options: OutputOptions, nested_grid=None):
        wind_step = '1 HR'  # TODO automate
        wind_drag = 1.0
        int_step = task.integration_step  # TODO automate

        config = SwanConfig(wind_step, wind_drag, int_step, grid, task, output_options=output_options, nested_grid=nested_grid)
    ...
    def fill_template(self, template: str):
        return template.format(coordinates_type=self.coordinates_type,
                               ...
                               hotstart=self._get_hotstart_description(),
                               ...
                               nested_output=self._get_nested_grid_description(),
                               hotfile_output=self._get_hotfile_description()
                               )
    ...
    def _get_model_params(self):
        if self.task.model == 'SWAN4072':
            phys_params = f"GEN3 KOMEN 2.36e-5 3.02e-3 \n" \
                          ...
                          "DIFFRACtion\n" \
                          "PROP BSBT\n"
        # TODO make sbatch a separate option
    ...
    def _get_nested_grid_description(self):
        if not self.nested_grid:
            return ''  # No nested grid, return empty string
    
        nested_descr = f"NGRID 'NESTGRID' {self.nested_grid.min_x} {self.nested_grid.min_y} 0. &\n" \
                       f"{self.nested_grid.x_len} {self.nested_grid.y_len} " \
                       f"{self.nested_grid.x_cells - 1} {self.nested_grid.y_cells - 1} \n \n"

        nested_descr += f"NESTout 'NESTGRID' 'results\\NESTOUT' " \
                        f"OUT {self.wind_start} 1 HR\n"
    
        return nested_descr
    
    def _get_hotstart_description(self):
        if not self.task.is_hotstart:
            return ''
        ## TODO allow for multiple or single hotstart files
        hotstart_descr = "INITial HOTStart MULTiple 'hotstart'"
    
        return hotstart_descr
    
    def _get_hotfile_description(self):
        if not self.output_options.is_save_hotfile:
            return ''
    
        hotfile_descr = "HOTFile 'hotstart'"
    
        return hotfile_descr
    ...
    def _get_bdc_description(self):
        ...
        if self.grid.bdc_description == 'nesting':
            nestout_path = './storage/NESTOUT'
            
            if not os.path.exists(nestout_path):
                raise FileNotFoundError(f"Error: Required nesting output file '{nestout_path}' is missing.")
        
            bdc_descr = f"BOUndnest1 NEst '{nestout_path}' CLOSed\n"
        else:
            if not self.grid.open_boundaries or not self.grid.bdc_description:
                return ''
            bdc_descr = 'BOUnd SHAPespec JONswap 3.3 PEAK DSPR DEGRees \n\n'
