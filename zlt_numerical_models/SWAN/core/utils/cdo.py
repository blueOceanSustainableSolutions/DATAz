...
def interpolate_to_grid(grid_path: str, source_path: str, target_path: str, nearest=False):
    ...
    if nearest:
        method = 'remapnn'
    cdo_executable = shutil.which("cdo")  # Path to CDO in cdoenv

    env = {"REMAP_EXTRAPOLATE": "on"}
    result = subprocess.run(
        [cdo_executable, "-P", "4", f"{method},{grid_path}", source_path, target_path],
        env=env,
        capture_output=True,
        text=True
    )
...
def grb2_to_nc(grid_path: str, nc_path: str):
    print(os.system(f'cdo -f nc copy '
...
def merge_time(template_to_merge: str, final_path: str):
    ...
    print(os.system(f'cdo mergetime {_process_local_path(template_to_merge)} {_process_local_path(final_path)}'))
...
def split_grib2(file_name: str):
    print(os.system(f'grib_copy {_process_local_path(file_name)} '
